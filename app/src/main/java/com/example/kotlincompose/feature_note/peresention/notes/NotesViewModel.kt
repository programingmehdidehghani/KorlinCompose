...
import com.example.kotlincompose.feature_note.domain.use_case.NoteUseCases
...
class NotesViewModel @Inject constructor(
    ...
    init {
        getNotes(NoteOrder.Date(OrderType.Dscending))
    }
    ...
    fun onEvent(event: NotesEvent){
        when(event){
           ...
           is NotesEvent.RestoreNote ->{
              viewModelScope.launch {
                  noteUseCases.addNote(recentlyDeleteNotes ?: return@launch)
                  recentlyDeleteNotes = null
              }
           }
           is NotesEvent.TaggleOrderSection -> {
